...
function PrimaryBtn({
  ...
  return (
    <button
      ...
      className="w-full inline-flex items-center justify-center gap-2 font-semibold rounded-xl transition-all duration-150 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed bg-brand-700 text-white hover:bg-brand-800 px-6 py-3 text-sm shadow-brand"
    >
      {icon}
...
function SecondaryBtn({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled || loading}
      className="w-full inline-flex items-center justify-center gap-2 font-semibold rounded-xl transition-all duration-150 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed bg-white text-brand-700 border-2 border-brand-200 hover:bg-brand-50 px-6 py-2.5 text-sm"
    >
      {icon}
      {loading ? 'Please wait…' : children}
    </button>
...
function PartnerWorkflow({
  ...
  if (isAssigned) {
    return (
      <div className="bg-white rounded-xl border border-gray-100 shadow-card p-5 mt-4">
        <p className="text-sm font-bold text-gray-900 mb-3">Delivery Steps</p>
        <PrimaryBtn
          onClick={() =>
            act('start', async () => {
...
export default function OrderDetail() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-brand-600 transition-colors mb-4 group"
      >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${s.cls}`}>
          {s.label}
        </span>
      </div>
      ...
      {order.status !== 'CANCELLED' && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-card p-5 mb-4">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-6">Order Progress</p>
          <div className="relative flex items-start justify-between px-2">
            <div
              className="absolute h-[2px] bg-gray-200 rounded-full"
              style={{ top: '16px', left: '24px', right: '24px' }}
            />
            <div
              className="absolute h-[2px] bg-brand-600 rounded-full transition-all duration-500"
              style={{
                top: '16px',
                left: '24px',
                width: cur <= 0
              ...
              return (
                <div key={step.key} className="flex flex-col items-center relative z-10" style={{ width: '20%' }}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
                    ...
                    {done    ? <CheckCircle2 size={15} className="text-white" strokeWidth={2.5} /> :
      ...
      {order.paymentMethod === 'CASH' &&
        ...
        order.status !== 'PLACED' &&
        order.status !== 'CANCELLED' && (
          <div className="mb-4 px-4 py-3 bg-amber-50 border border-amber-100 rounded-xl text-sm text-amber-800">
            {dbRetryCount >= 3
      ...
      {/* Detail cards — tighter, side-by-side on md+ */}
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-card p-4">
          ...
          {order.payment?.status === 'PENDING' && order.paymentMethod === 'UPI' && (
            <div className="mt-3 pt-3 border-t border-gray-100">
              ...
              <p className="text-sm font-bold text-amber-600">Pending Payment</p>
              <p className="text-xs text-gray-400 mt-0.5">Method: UPI</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {canPayUpi && (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 font-semibold rounded-xl transition-all duration-150 active:scale-95 bg-brand-600 text-white hover:bg-brand-700 shadow-brand px-5 py-2.5 text-sm"
            >
              Pay UPI
            </button>
          )}
          {invoiceReady && (
            <>
              <button
                ...
                className="inline-flex items-center justify-center gap-2 font-semibold rounded-xl transition-all duration-150 active:scale-95 bg-white text-brand-700 border-2 border-brand-200 hover:bg-brand-50 px-5 py-2.5 text-sm"
              >
                <FileText size={13} />
                View Invoice
              </button>
          ...
          {canCancel && (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 font-semibold rounded-xl transition-all duration-150 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed bg-white text-red-600 border-2 border-red-200 hover:bg-red-50 px-5 py-2.5 text-sm"
            >
...
function DetailRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between py-0.5">
      ...
      <dd className="text-xs font-medium text-gray-900">{children}</dd>
    </div>
  );
}