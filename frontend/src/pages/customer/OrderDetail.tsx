...
export default function OrderDetail() {
  ...
  // Show refund section only when a payment record exists
  // with a refundStatus that is not NOT_REQUIRED
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Refund Section — shown only when a UPI refund is in progress or completed */}
        ...
        <Card className="mt-5 border border-yellow-100 bg-yellow-50/40">
          ...
          <dl className="space-y-2.5 text-sm">
            ...
            {order.payment.refundInitiatedAt && (
              <Row label="Initiated At">
                {fmtDateTime(order.payment.refundInitiatedAt)}
              </Row>
            )}

            {order.payment.refundCompletedAt && (
              <Row label="Completed At">
                {fmtDateTime(order.payment.refundCompletedAt)}
              </Row>
            )}
